...
- [x] **Phase 2 — Ingestion connector & queue.** S3 ObjectCreated → SQS; connector pulls audio + metadata and enqueues work items with idempotency, retries, DLQ. *(Done 2026-06-11: local Postgres via `docker-compose.yml` (Colima engine), schema auto-applied. SQS `acx-recordings` + DLQ `acx-recordings-dlq` (redrive maxReceiveCount=5, 60s visibility, 20s long-poll) via `infra/aws/provision_sqs.py`. `s3:ObjectCreated:*` on `CallRecordings/` prefix → SQS via `provision_s3_notification.py` (queue resource policy + confused-deputy conditions). Least-privilege IAM user `acx-connector` via `provision_connector_iam.py` (scoped SQS consume + `s3:GetObject` on recording prefixes; verified denied `ListQueues`). Connector in `src/connector/{consumer,db}.py`: long-poll → skip `s3:TestEvent` → parse contact_id → HeadObject → upsert interaction anchor → idempotent `media_record` insert (status `queued`) → commit → delete. Two-tier creds (admin provisions, connector runs). Tested live via `infra/aws/upload_test_recording.py`; idempotency confirmed; `sql/validation/phase2_checks.sql` green. Capstone (2026-06-12): refactored the ingest core into shared `ingest_object()` and added `src/connector/backfill.py` — lists the recordings prefix and pulls existing/historical objects through the same idempotent path (forward-only events leave history stranded; this is the onboarding/reconciliation motion). Backfilled the 6 real Phase-1 contacts (2 legs each); re-run is a clean no-op. DB now holds 13 `media_records`.)*
